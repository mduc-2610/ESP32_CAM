...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function cancelRecognition() {
    ...
    photoStatus.textContent = "Đã hủy nhận diện, bắt đầu stream lại.";
    ...
    // Restart stream by setting src and showing it
    if (cameraStream) {
      cameraStream.src = `http://${cameraIp}/stream`;
      cameraStream.style.display = "block";
    }
  ...
  function handleCaptureError(error) {
    ...
    // Restart stream in case of error
    if (cameraStream) {
      cameraStream.src = `http://${cameraIp}/stream`;
      cameraStream.style.display = "block";
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.attendance) {
          ...
          photoStatus.textContent = "Nhận diện hoàn tất nhưng không đủ điều kiện để điểm danh.";
      ...
      // Stream remains hidden, user must click refresh to show stream again
    }
  }
